...
<!-- Overlay для каталогу — вся сторінка -->
{#if catalogOpen}
  <button
    transition:fade={{ duration: 150 }}
    class="fixed inset-0 bg-black/30 z-40 cursor-default w-full border-none p-0"
    onclick={() => (catalogOpen = false)}
    aria-label="Закрити каталог"
  ></button>
{/if}

<!-- Overlay для пошуку — вся сторінка -->
{#if searchOpen}
  <button
    transition:fade={{ duration: 150 }}
    class="fixed inset-0 bg-black/20 z-40 cursor-default w-full border-none p-0"
...
>
  ...
  >
    <div
      class="max-w-7xl mx-auto h-16 px-4 sm:px-6 grid grid-cols-[auto_1fr_auto] items-center gap-4"
    >
      ...
      <UserMenu {role} {user} hasNotifications={true} onnavigate={navigate} />
    </div>
  </header>
